	...
	public void book() throws InterruptedException, IOException {
		try {
			...
			if(book.isDisplayed()) {
				...
				try {
					test.info("Click on OrderComplete");
					if(orderComplete.isDisplayed()) {
				orderComplete.click();
				Thread.sleep(5000);
				passCase("OrderComplete is done");
				
				
			}
				}
				catch(Exception e) {
					failCase("OrderComplete was not locateable. Please check the error message", "OrderCompletefail");
